...
用 Go 编写的 MCP 搜索服务，内置百度网页搜索、Bing、DuckDuckGo 等通用搜索引擎和 9 个学术搜索引擎。可作为 MCP 工具接入 Claude Code、Qwen Code、Cursor 等客户端，也可作为 Go 模块嵌入自有服务。
...
**面向 LLM 的完整工具链** — 4 个工具覆盖联网工作流：`smartsearch` 搜索（可选 LLM 结构化摘要）→ `academicsearch` 学术检索 → `cleanfetch` 抓取原文 → `pdf_parser` 解析 PDF，结果互相衔接，一次配置全链路可用。
...
# 
#    首次 install 会在可执行文件目录自动生成一份可编辑的预设 config.yaml 和autostart.vbs
...
| 多引擎融合 | 6 种搜索模式、7 个通用引擎 + 9 个学术引擎，主引擎失败自动回退 |
| 相关性评分 | RRF 融合排名 + 词汇对齐 / 域名品质 / 共识 / 权威 / 时效加分，低分结果自动裁剪；MMR 打散同主题高相似结果（转载 / 镜像站） |
| 学术搜索 | 9 大学术引擎并行，按引用数 / 期刊权威 / PDF 可用性 / 新鲜度评分；DOI 跨引擎去重，单引擎失败透传不影响整体 |
| 网页抓取 | `cleanfetch` 无需代理抓取网页，内置 SSRF / DNS rebinding 防护与超大文件预检 |
| PDF 解析 | 本地 PDF 文本优先提取，扫描件可回退 MinerU OCR |
| LLM 摘要 | 可选接入 OpenAI 兼容 API 生成结构化摘要，支持流式推送生成过程 |
| 系统代理自动检测 | Clash 等开启系统代理后，海外引擎 / Jina Reader 自动走代理 |
...
| `apipool` | API Key 池轮转：每次只调一个供应商，失败自动切换，支持 round-robin / priority / weighted 加权策略 | 各 Key 可选 |
