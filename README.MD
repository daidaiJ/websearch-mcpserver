...
```bash
go build -o websearch ./cmd/main.go
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `url` | string | ✅ | 网页 URL（需配置 `jina.api_key`） |

> 工具注册条件：`smartsearch` 需 `bing.enabled=true`；`academicsearch` 需 `academic.enabled=true`；`cleanfetch` 需配置 `jina.api_key`。
